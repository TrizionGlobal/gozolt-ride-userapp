...
import 'bike_rental_cancellation_success_screen.dart';
import 'rental_extension_success_screen.dart';

final bikeRentalBookingDetailsProvider = FutureProvider.autoDispose.family<Map<String, dynamic>, String>((ref, bookingId) async {
  final dio = ref.read(dioProvider);
  final datasource = BikeRentalRemoteDatasource(dio);
  return datasource.getBookingDetails(bookingId);
});

class BikeRentalBookingDetailsScreen extends ConsumerWidget {
  final String bookingId;

  const BikeRentalBookingDetailsScreen({super.key, required this.bookingId});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final asyncDetails = ref.watch(bikeRentalBookingDetailsProvider(bookingId));
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          final vehicleName = vehicle['name'] ?? 'Premium Bike';

          final brand = vehicle['brand'] ?? '-';
          final model = vehicle['model'] ?? '-';
          final year = vehicle['manufacturingYear']?.toString() ?? '-';
          final type = vehicle['type'] ?? '-';
          final registrationNumber = vehicle['registrationNumber'] ?? '-';
          final engineCc = vehicle['engineCapacityCc']?.toString() ?? '-';
          final fuelType = vehicle['fuelType'] ?? '-';
          final transmission = vehicle['transmission'] ?? '-';
          final mileage = vehicle['mileage']?.toString() ?? '-';
...
Seats: $seats
Color: $color
Pickup Date: ${DateFormat('MMM dd, yyyy').format(startDate)}
Pickup Time: ${DateFormat('h:mm a').format(startDate)}
...
'''.trim();
          ...
          return RefreshIndicator(
            ...
            onRefresh: () => ref.refresh(bikeRentalBookingDetailsProvider(bookingId).future),
            child: SingleChildScrollView(
              ...
              children: [
                ...
                // Vehicle Details
                _buildSectionTitle(context, 'Bike Details'),
                const SizedBox(height: 8),
                _buildInfoCard(context, [
                  _buildInfoRow(context, 'Bike', vehicle['name'] ?? 'N/A'),
                  ...
                  if (vehicle['engineCapacityCc'] != null) _buildInfoRow(context, 'Engine (CC)', '${vehicle['engineCapacityCc']}cc'),
                ...
                _buildInfoCard(context, [
                  ...(() {
                    ...
                    final rows = <Widget>[
                      _buildInfoRow(context, 'Bike Rate', '€${vehicleRate.toStringAsFixed(2)}'),
                    ];
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                ...
                ref.invalidate(bikeRentalBookingDetailsProvider(booking['id']));
                ...
                if (context.mounted) {
                  ...
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) => BikeRentalCancellationSuccessScreen(refundAmount: finalRefund),
                    ),
                  );
                }
              } catch (e) {
  ...
  Widget _buildExtendConfirmationModal(
    ...
    BikeRentalRemoteDatasource datasource,
                          ...
                          builder: (_) => StripeAddCardSheet(
                            ...
                            onCardAdded: (paymentMethodId) async {
                              ...
                              try {
                                ...
                                ref.invalidate(bikeRentalBookingDetailsProvider(bookingId));
