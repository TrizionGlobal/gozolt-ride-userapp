...
class PlumbingDetailsScreen extends ConsumerStatefulWidget {
  final QuickServiceBookingData bookingData;

  const PlumbingDetailsScreen({super.key, required this.bookingData});

  @override
  ConsumerState<PlumbingDetailsScreen> createState() => _PlumbingDetailsScreenState();
}

class _PlumbingDetailsScreenState extends ConsumerState<PlumbingDetailsScreen> {
  ...
  final Map<String, IconData> _icons = {
    'Leaking Tap': Icons.water_drop,
    'Blocked Drain': Icons.waves,
    'Pipe Leakage': Icons.plumbing,
    'Toilet Repair': Icons.bathroom,
                      ...
                      onPressed: () {
                        ...
                        if (_selectedImages.isNotEmpty) {
                          ...
                          ref.read(quickServicesBookingProvider.notifier).uploadImages(
                            ...
                            if (mounted) context.pushNamed(RouteNames.quickServicesPlumbingReview, extra: finalDataObj);
                          }).catchError((e) {
                            if (mounted) setState(() => _isUploading = false);
                            if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to upload images')));
                          });
                        } else {
                          if (mounted) context.pushNamed(RouteNames.quickServicesPlumbingReview, extra: updatedData);
                        }
                      },
