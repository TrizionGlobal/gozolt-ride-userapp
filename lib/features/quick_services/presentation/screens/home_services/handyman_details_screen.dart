...
class HandymanDetailsScreen extends ConsumerStatefulWidget {
  final QuickServiceBookingData bookingData;

  const HandymanDetailsScreen({super.key, required this.bookingData});

  @override
  ConsumerState<HandymanDetailsScreen> createState() => _HandymanDetailsScreenState();
}

class _HandymanDetailsScreenState extends ConsumerState<HandymanDetailsScreen> {
  ...
  void _onContinue() {
    ...
    if (_selectedImages.isNotEmpty) {
      ...
      ref.read(quickServicesBookingProvider.notifier).uploadImages(
        ...
        if (mounted) context.pushNamed(RouteNames.quickServicesHandymanReview, extra: finalDataObj);
      }).catchError((e) {
        if (mounted) setState(() => _isUploading = false);
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to upload images')));
      });
    } else {
      if (mounted) context.pushNamed(RouteNames.quickServicesHandymanReview, extra: updatedData);
    }
  }
