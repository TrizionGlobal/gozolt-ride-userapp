...
class LiftElevatorMechanicDetailsScreen extends ConsumerStatefulWidget {
  final QuickServiceBookingData bookingData;

  const LiftElevatorMechanicDetailsScreen({super.key, required this.bookingData});

  @override
  ConsumerState<LiftElevatorMechanicDetailsScreen> createState() => _LiftElevatorMechanicDetailsScreenState();
}

class _LiftElevatorMechanicDetailsScreenState extends ConsumerState<LiftElevatorMechanicDetailsScreen> {
  ...
  void _onContinue() {
    ...
    if (_selectedImages.isNotEmpty) {
      ...
      ref.read(quickServicesBookingProvider.notifier).uploadImages(
        ...
        if (mounted) context.pushNamed(RouteNames.quickServicesLiftElevatorReview, extra: finalDataObj);
      }).catchError((e) {
        if (mounted) setState(() => _isUploading = false);
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to upload images')));
      });
    } else {
      if (mounted) context.pushNamed(RouteNames.quickServicesLiftElevatorReview, extra: updatedData);
    }
  }
