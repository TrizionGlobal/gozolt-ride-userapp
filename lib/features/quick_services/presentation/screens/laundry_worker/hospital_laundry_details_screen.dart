...
class HospitalLaundryDetailsScreen extends StatefulWidget {
  final QuickServiceBookingData bookingData;

  const HospitalLaundryDetailsScreen({super.key, required this.bookingData});

  @override
  State<HospitalLaundryDetailsScreen> createState() => _HospitalLaundryDetailsScreenState();
}

class _HospitalLaundryDetailsScreenState extends State<HospitalLaundryDetailsScreen> {
  ...
  final List<String> _availableLaundryTypes = ['Bed Linen', 'Patient Gowns', 'Towels', 'Staff Uniforms', 'Other Linen'];
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Hospital laundry is collected and returned by an approved provider.',
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        // 1. Facility Details
                        _buildSectionHeader('1. Facility Details'),
                        Row(
                          children: [
                            Expanded(child: _buildTextField('Facility Name', _facilityNameController)),
                        ...
                        if (_selectedLaundryTypes.contains('Other Linen')) ...[
                          const SizedBox(height: 12),
                          _buildTextField('', _otherLaundryTypeController, hintText: 'Please specify other linen type'),
                        ],
                        const Divider(height: 32),
                        ...
                        // 4. Linen Type
                        _buildSectionHeader('4. Linen Type'),
                        Wrap(
                          spacing: 12,
                          runSpacing: 12,
                          children: ['Standard Linen', 'Special Handling Required', 'Not Sure'].map((type) {
                        ...
                        Text(
                          'The provider will confirm handling requirements before collection.',
                          style: AppTextStyles.bodySmall.copyWith(color: Colors.grey),
                        ),
                        const Divider(height: 32),

                        // 5. Requested Return
                        _buildSectionHeader('5. Requested Return'),
                        Row(
                          children: [
                            Expanded(
                        ...
                        _buildSectionHeader('6. Additional Details'),
                        ...
                        ElevatedButton(
                          onPressed: () {
                            if (_facilityNameController.text.trim().isEmpty || _collectionPointController.text.trim().isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please fill all required facility details.')));
                              return;
                            }
                            if (_linenType == null) {
                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a linen type.')));
                              return;
                            }
                            ...
                            if (finalLaundryTypes.contains('Other Linen') && _otherLaundryTypeController.text.trim().isNotEmpty) {
                              finalLaundryTypes.remove('Other Linen');
                              finalLaundryTypes.add(_otherLaundryTypeController.text.trim());
                            }

                            final updatedData = widget.bookingData.copyWith(
                              selectedServiceTitle: 'Hospital Laundry',
