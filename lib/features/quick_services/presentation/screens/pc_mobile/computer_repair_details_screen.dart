...
class ComputerRepairDetailsScreen extends ConsumerStatefulWidget {
  final QuickServiceBookingData bookingData;

  const ComputerRepairDetailsScreen({super.key, required this.bookingData});

  @override
  ConsumerState<ComputerRepairDetailsScreen> createState() => _ComputerRepairDetailsScreenState();
}

class _ComputerRepairDetailsScreenState extends ConsumerState<ComputerRepairDetailsScreen> {
                    ...
                    onPressed: () {
                      ...
                      if (_selectedImages.isNotEmpty) {
                        ...
                        ref.read(quickServicesBookingProvider.notifier).uploadImages(
                          ...
                          if (mounted) context.pushNamed(RouteNames.quickServicesComputerRepairReview, extra: finalDataObj);
                        }).catchError((e) {
                          if (mounted) setState(() => _isUploading = false);
                          if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to upload images')));
                        });
                      } else {
                        if (mounted) context.pushNamed(RouteNames.quickServicesComputerRepairReview, extra: updatedData);
                      }
                    },
