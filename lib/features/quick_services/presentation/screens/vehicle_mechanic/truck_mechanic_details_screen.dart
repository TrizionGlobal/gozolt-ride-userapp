...
class TruckMechanicDetailsScreen extends ConsumerStatefulWidget {
  final QuickServiceBookingData bookingData;
  const TruckMechanicDetailsScreen({super.key, required this.bookingData});

  @override
  ConsumerState<TruckMechanicDetailsScreen> createState() => _TruckMechanicDetailsScreenState();
}

class _TruckMechanicDetailsScreenState extends ConsumerState<TruckMechanicDetailsScreen> {
                    ...
                    onPressed: () {
                      ...
                      if (_selectedImages.isNotEmpty) {
                        ...
                        ref.read(quickServicesBookingProvider.notifier).uploadImages(
                          ...
                          if (mounted) context.pushNamed(RouteNames.quickServicesTruckMechanicReview, extra: finalDataObj);
                        }).catchError((e) {
                          if (mounted) setState(() => _isUploading = false);
                          if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to upload images')));
                        });
                      } else {
                        if (mounted) context.pushNamed(RouteNames.quickServicesTruckMechanicReview, extra: updatedBookingData);
                      }
                    },
